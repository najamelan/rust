...
impl<'hir> Map<'hir> {
    ...
    pub fn name_by_hir_id(&self, id: HirId) -> Name {
        match self.get_by_hir_id(id) {
            ...
            Node::StructCtor(_) => self.name_by_hir_id(self.get_parent_item(id)),
    ...
    pub fn attrs_by_hir_id(&self, id: HirId) -> &'hir [ast::Attribute] {
        ...
        let attrs = match self.find_by_hir_id(id) {
            ...
            Some(Node::StructCtor(_)) => return self.attrs_by_hir_id(self.get_parent_item(id)),
            _ => None
        };
        attrs.unwrap_or(&[])
    }
    ...
    pub fn span_by_hir_id(&self, hir_id: HirId) -> Span {
        ...
        match self.find_entry(hir_id).map(|entry| entry.node) {
            ...
            Some(Node::StructCtor(_)) => self.expect_item_by_hir_id(
                self.get_parent_item(hir_id)).span,
